...
static std::vector<std::vector<int>> parseAdjList(const std::string& s) {
    std::string t;
    t.reserve(s.size());
    ...
    if (t.size() < 2 || t.front() != '[' || t.back() != ']')
        throw std::runtime_error("Invalid format: must start with '[' and end with ']'");
    ...
    auto parseInt = [&]() -> int {
        if (i >= (int)t.size() || (!std::isdigit((unsigned char)t[i]) && t[i] != '-'))
            throw std::runtime_error("Parse error: expected integer");
        int sign = 1;
        if (t[i] == '-') { sign = -1; ++i; }
        long long val = 0;
        if (i >= (int)t.size() || !std::isdigit((unsigned char)t[i]))
            throw std::runtime_error("Parse error: invalid integer");
        while (i < (int)t.size() && std::isdigit((unsigned char)t[i])) {
            val = val * 10 + (t[i] - '0');
            ++i;
        }
        return (int)(sign * val);
    ...
    if (i != (int)t.size())
        throw std::runtime_error("Parse error: trailing characters");
...
static Node* buildGraphFromAdj(const std::vector<std::vector<int>>& adj) {
    if (adj.empty()) return nullptr;

    int n = (int)adj.size();
    std::vector<Node*> nodes(n + 1, nullptr); // 1..n

    for (int i = 1; i <= n; ++i) nodes[i] = new Node(i);

    for (int i = 1; i <= n; ++i) {
        for (int v : adj[i - 1]) {
            if (v < 1 || v > n) throw std::runtime_error("Neighbor label out of range");
...
static std::vector<std::vector<int>> serializeGraphToAdj(Node* start) {
    if (!start) return {};
    ...
    std::unordered_map<int, Node*> labelToNode;
    labelToNode[start->val] = start;

    while (!q.empty()) {
        Node* cur = q.front(); q.pop();
        maxLabel = std::max(maxLabel, cur->val);
        labelToNode[cur->val] = cur;

        for (Node* nei : cur->neighbors) {
            if (!nei) continue;
            maxLabel = std::max(maxLabel, nei->val);
    ...
    std::vector<std::vector<int>> adj(maxLabel);
...
static void printAdj(const std::vector<std::vector<int>>& adj) {
    std::cout << "[";
...
static void runExample(const std::string& input) {
    std::cout << "\n========================================\n";
    std::cout << "Input:  adjList = " << input << "\n";
    ...
    std::cout << "Output (original serialized): ";
    printAdj(origAdj);
    std::cout << "\n";

    std::cout << "Output (clone serialized):    ";
    printAdj(cloneAdj);
    std::cout << "\n";

    std::cout << "Deep copy check: " << (deepCopyCheck(start, cloned) ? "PASS" : "FAIL") << "\n";
}
